...
def calculate_probs_advanced (known_board):
    ...
    for field5 in free_fields_lvl_0:
        ...
        for orient in orientations:
            ...
            if fun.n_segments(board_hyp_lvl_0, 5) == 5 or game_rules_n_ships[5] == 0:
                mat = True
            ...
            for field4 in free_fields_lvl_0:
                ...
                for orient in orientations:
                    ...
                    if fun.n_segments(board_hyp_lvl_1, 4) == 4 or game_rules_n_ships[4] == 0:
                        mat = True
                    ...
                    for field31 in free_fields_lvl_0:
                        ...
                        for orient in orientations:
                            ...
                            if fun.n_segments(board_hyp_lvl_2, 3) >= 3 or game_rules_n_ships[3] < 2:
                                mat = True
                            ...
                            for field32 in free_fields_lvl_0:
                                ...
                                for orient in orientations:
                                    if seg32_already_placed:
                                        break
                                    # If board already contains all possible '3' fields -> go on and use unmodified previous level board
                                    if fun.n_segments(board_hyp_lvl_3, 3) == 6 or game_rules_n_ships[3] == 0:
                                        mat = True
                                    ...
                                    for field21 in free_fields_lvl_0:
                                        ...
                                        for orient in orientations:
                                            ...
                                            if fun.n_segments(board_hyp_lvl_4, 2) >= 2 or game_rules_n_ships[2] < 3:
                                                mat = True
                                            ...
                                            for field22 in free_fields_lvl_0:
                                                ...
                                                for orient in orientations:
                                                    if seg22_already_placed:
                                                        break
                                                    # If board already contains all possible '2' fields -> go on and use unmodified previous level board
                                                    if fun.n_segments(board_hyp_lvl_5, 2) >= 4 or game_rules_n_ships[2] < 2:
                                                        mat = True
                                                    ...
                                                    for field23 in free_fields_lvl_0:
                                                        ...
                                                        for orient in orientations:
                                                            if seg23_already_placed:
                                                                break
                                                            # If board already contains all possible '2' fields -> go on and use unmodified previous level board
                                                            if fun.n_segments(board_hyp_lvl_6, 2) == 6 or game_rules_n_ships[2] == 0:
    ...
    occurances = np.zeros((dim, dim), dtype = int)
    occurances_seg3 = np.zeros((dim, dim), dtype = int)
    occurances_seg2 = np.zeros((dim, dim), dtype = int)
                ...
                if field not in hit_unsunk:
                    for ar in chunk:
                        if ar[x][y] > 1 and ar[x][y] < 7:
                            occurances[x][y] += 1
                        if ar[x][y] == 3:
                            occurances_seg3[x][y] += 1
                        if ar[x][y] == 2:
                            occurances_seg2[x][y] += 1
        w += 1
    ...
    return occurances, best_field, best_prob, total_unique, time_adv, board_state_100chars_code, occurances_seg3, occurances_seg2
