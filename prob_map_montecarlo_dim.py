...
def count_occurances(board: np.ndarray, free_fields: list, hit_unsunk: list, occurances: np.ndarray, weight: float = 1.0)\
                    -> np.ndarray:
    """
...
def calculate_confidence(counts: dict, generated_games: int, conf_level: float) -> dict:
    ...
    for num, count in counts.items():
        fraction = min(1, count / generated_games)
...
# MAIN FUNCTION
###########################################################################
def calculate_probs_montecarlo (known_board, conf_level = 2.58, margin_estim = 1.00, margin_highest = 0.12, bias_detected = False, class_weights = None, clf = None, scaler_params = None):
    start_time = time.time()
    orientations = ('hor', 'ver')
    generated_games = 0
    limiter = "dummy"

    # Create 1D list of 64 fields and convert to tuple (0 to 63)
    fields = tuple([(x, y) for x in range(dim) for y in range(dim)])

    # Create 2D np.array of 64 fields to count occurances for each field
    occurances = np.zeros((dim, dim), dtype = float)
...
if __name__ == "__main__":
    ...
    # known_board[0] = [0, 0, 0, 0, 0, 7, 7, 3]
    # known_board[1] = [7, 0, 0, 0, 7, 7, 7, 3]
    # known_board[2] = [7, 0, 0, 0, 7, 7, 7, 3]
    # known_board[3] = [7, 0, 0, 0, 7, 7, 7, 7]
    # known_board[4] = [7, 0, 0, 0, 7, 7, 7, 7]
    # known_board[5] = [7, 0, 0, 0, 7, 7, 7, 7]
    # known_board[6] = [0, 0, 0, 0, 0, 0, 0, 0]
    # known_board[7] = [0, 0, 0, 0, 0, 0, 0, 0]
